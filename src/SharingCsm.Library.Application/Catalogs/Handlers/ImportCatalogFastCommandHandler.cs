...
public sealed record ImportCatalogFastCommand(Stream Stream) : ICommand;

public class ImportCatalogFastCommandHandler : ICommandHandler<ImportCatalogFastCommand>
{
	private readonly IBookRepository _bookRepository;
	private readonly ICatalogImportService _catalogImportService;
	private readonly UnitOfWork _unitOfWork;

	public ImportCatalogFastCommandHandler(IBookRepository bookRepository, ICatalogImportService catalogImportService, UnitOfWork unitOfWork)
	{
	...
	public async  ValueTask<Unit> Handle(ImportCatalogFastCommand command, CancellationToken cancellationToken)
	{
		var booksBatch = new List<Book>(1000);

		await foreach (var dao in _catalogImportService.ImportFastAsync(command.Stream, cancellationToken))
		{
